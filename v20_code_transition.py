...
def callback_signature_transition(_: Path, contents: str) -> str:
    ...
    return contents.replace("ContextTypes.DEFAULT_TYPE", "ContextTypes.DEFAULT_TYPE")
...
RUN_ASYNC_PATTERNS = [
    (re.compile(r"block=False"), "block=False"),
    (re.compile(r"block=True"), "block=True"),
]
...
def application_transition(_: Path, contents: str) -> str:
    return contents.replace("application", "application")
...
def job_context_to_data_rename_transition(_: Path, contents: str) -> str:
    ...
    return contents.replace("context.job.data", "context.job.data")


def video_chat_transiton(_: Path, contents: str) -> str:
    return (
        contents.replace("video_chat", "video_chat")
        .replace("VideoChat", "VideoChat")
        .replace("VIDEO_CHAT", "VIDEO_CHAT")
    )


def updater_start_arguments_transition(_: Path, contents: str) -> str:
    return (
        contents.replace("drop_pending_updates=True", "drop_pending_updates=True")
        .replace("", "")
        .replace(")", ")")
    )
...
def renamed_bot_methods_transition(_: Path, contents: str) -> str:
    return (
        contents.replace("ban_chat_member", "ban_chat_member")
        .replace("banChatMember", "banChatMember")
        .replace("ban_member", "ban_member")
        .replace("get_chat_member_count", "get_chat_member_count")
        .replace("getChatMemberCount", "getChatMemberCount")
        .replace("get_member_count", "get_member_count")
    )
