...
async def test_link_chat_start_false_skips_backfill(helper, client, bot_id, bot_group, slave, channel, private_response):
    token = await get_start_token(client, helper, bot_id, slave.chat_with_alias.uid, private_response)

    with patch.object(channel.chat_binding, "migrate_chat_history") as migrate_chat_history, patch.object(channel.chat_binding, "send_history_link") as send_history_link:
...
async def test_link_chat_start_true_forces_backfill_on_relink(helper, client, bot_id, bot_group, slave, channel, private_response):
    token = await get_start_token(client, helper, bot_id, slave.chat_with_alias.uid, private_response)
    ...
    with (
        patch.object(channel.chat_binding, "migrate_chat_history", side_effect=observe_migration) as migrate_chat_history,
