...
def _manager():
    ...
    manager._author_avatar_lock = threading.Lock()
...
def test_author_avatar_lazy_cache_miss_starts_background_create_without_loading_picture():
    manager = _manager()
    load_picture = Mock(side_effect=AssertionError("avatar should not be loaded"))
    thread = Mock()
...
def test_author_avatar_lazy_inflight_user_does_not_start_duplicate_background_create():
    manager = _manager()
    manager._author_avatar_inflight.add(manager._author_avatar_cache_key("slave user"))
...
def test_author_avatar_background_create_writes_user_cache():
    manager = _manager()
    manager._get_or_create_topic_icon_custom_emoji_entry = Mock(return_value=("12345", "set_by_bot"))

    manager._load_author_avatar_custom_emoji(
        "slave user",
    ...
    manager._get_or_create_topic_icon_custom_emoji_entry.assert_called_once()
...
def test_author_avatar_background_create_clears_inflight_after_success():
    manager = _manager()
    user_key = manager._author_avatar_cache_key("slave user")
    ...
    assert user_key not in manager._author_avatar_inflight
...
def test_author_avatar_background_create_clears_inflight_after_failure():
    manager = _manager()
    user_key = manager._author_avatar_cache_key("slave user")
    manager._author_avatar_inflight.add(user_key)
    manager._get_or_create_topic_icon_custom_emoji_entry = Mock(side_effect=RuntimeError("telegram down"))
