...
def test_author_avatar_lazy_cache_miss_starts_background_create_without_loading_picture(channel, slave):
    slave_uid = f"{utils.chat_id_to_str(chat=slave.group.members[0])}:cache-miss"
...
def test_author_avatar_lazy_inflight_user_does_not_start_duplicate_background_create(channel, slave):
    slave_uid = f"{utils.chat_id_to_str(chat=slave.group.members[0])}:inflight"
    ...
    load_picture = Mock(side_effect=AssertionError("avatar should not be loaded while inflight"))
...
def test_author_avatar_background_create_adds_sticker_and_user_cache(channel, slave):
    slave_uid = f"{utils.chat_id_to_str(chat=slave.group.members[0])}:create"
    user_cache_key = channel.chat_binding._author_avatar_cache_key(slave_uid)
    picture = _png_bytes((200, 10, 10, 255))

    with patch.object(channel.chat_binding, "_get_or_create_topic_icon_custom_emoji_entry",
                      return_value=("emoji-author", "etm_topic_icons_by_testbot")) as create:
        channel.chat_binding._load_author_avatar_custom_emoji(
            slave_uid,
            user_cache_key,
...
def test_author_avatar_background_create_clears_inflight_when_create_fails(channel, slave):
    slave_uid = f"{utils.chat_id_to_str(chat=slave.group.members[0])}:create-fail"
    user_cache_key = channel.chat_binding._author_avatar_cache_key(slave_uid)
    ...
    assert user_cache_key not in channel.chat_binding._author_avatar_inflight
