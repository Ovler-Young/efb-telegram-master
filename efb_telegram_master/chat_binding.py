...
class ChatBindingManager(LocaleMixin):
    ...
    def update_group_info(self, update: Update, context: CallbackContext):
        ...
        if tg_chat.is_forum and len(chats) > 1:
            return self._update_forum_group_info(update, tg_chat)
    ...
    def _update_forum_group_info(self, update: Update, tg_chat):
        """
        Update forum group info for multiple linked chats.
        Send a message with avatar and description in each topic and pin it.
        """
        try:
            topic_slaves = self.db.get_topic_slaves(topic_chat_id=TelegramChatID(tg_chat.id))
            if not topic_slaves:
                return self.bot.reply_error(update, self._('No topics found in this forum group.'))
            ...
            if current_thread_id:
                ...
                if not topics_to_update:
                    return self.bot.reply_error(update, self._('This topic is not managed by this bot.'))
            else:
            ...
            for slave_uid, thread_id in topics_to_update:
                try:
                    success = self._update_single_topic_info(tg_chat.id, thread_id, slave_uid)
                    if success:
                        updated_count += 1
                except Exception as e:
            ...
            if updated_count > 0:
                update.effective_message.reply_text(
                    self.ngettext('Updated {count} topic.', 'Updated {count} topics.', updated_count)
                    .format(count=updated_count)
                )
            else:
                return self.bot.reply_error(update, self._('Failed to update any topics.'))
