...
class SlaveMediaDelivery:
    ...
    def animation(self, msg: Message, destination: TelegramChatID, thread_id: Optional[TelegramTopicID], template: str, reactions: str, old_message_id: Optional[OldMsgID] = None, reply_to: Optional[TelegramMessageID] = None, reply_markup: Optional[ReplyMarkup] = None, silent: Optional[bool] = None) -> telegram.Message:
        ...
        try:
            ...
            if old_message_id:
                ...
                if edit_media:
                    ...
                    if not text:
                        return sent
                return self.bot.edit_message_caption(chat_id=old_message_id[0], message_id=old_message_id[1], prefix=template, suffix=reactions, reply_markup=reply_markup, caption=text, parse_mode="HTML", **metadata)
            assert msg.file and msg.path
            ...
            return self.bot.send_animation(destination, animation, prefix=template, suffix=reactions, caption=text, parse_mode="HTML", reply_to_message_id=reply_to, message_thread_id=thread_id, reply_markup=reply_markup, disable_notification=silent, **send_identity(msg))
        finally:
            if msg.file is not None:
                msg.file.close()

    def sticker(self, msg: Message, destination: TelegramChatID, thread_id: Optional[TelegramTopicID], template: str, reactions: str, old_message_id: Optional[OldMsgID] = None, reply_to: Optional[TelegramMessageID] = None, reply_markup: Optional[ReplyMarkup] = None, silent: bool = False) -> telegram.Message:
        self.bot.send_chat_action(destination, ChatAction.UPLOAD_PHOTO, message_thread_id=thread_id)
        ...
        try:
            ...
            if old_message_id and not msg.edit_media:
                try:
                    ...
                    return self.bot.send_message(chat_id=old_message_id[0], reply_to_message_id=old_message_id[1], prefix=template, text=msg.text, suffix=reactions, reply_markup=reply_markup, disable_notification=silent)
            notice = self.file_transfer.check_size(msg.file)
            if notice:
                oversized, _ = self.notice_sender.send(msg, notice, destination, thread_id, template, reactions, self.text_delivery.html_substitutions(msg), old_message_id, reply_to, reply_markup, silent)
                if oversized is not None:
                    return oversized
            webp = None
            try:
                ...
                webp.seek(0)
                return self.bot.send_sticker(destination, self.file_transfer.prepare(webp, webp.name, msg.filename), reply_markup=sticker_markup, message_thread_id=thread_id, reply_to_message_id=reply_to, disable_notification=silent, **send_identity(msg))
            except IOError:
                self.logger.warning("[%s] Failed to convert image to webp sticker, sending as document.", msg.uid)
                assert msg.file and msg.path
                return self.bot.send_document(destination, self.file_transfer.prepare(msg.file, msg.path, msg.filename), prefix=template, suffix=reactions, message_thread_id=thread_id, caption=msg.text, filename=msg.filename, reply_to_message_id=reply_to, reply_markup=reply_markup, disable_notification=silent, **send_identity(msg))
            finally:
