...
from ehforwarderbot.exceptions import EFBException, EFBOperationNotSupported, EFBChatNotFound, EFBMessageReactionNotPossible
...
class TelegramChannel(MasterChannel):
    ...
    def __init__(self, instance_id: InstanceID = None):
        ...
        self.application.add_handler(CommandHandler("start", self.start, filters=non_edit_filter))
        self.application.add_handler(CommandHandler("help", self.help, filters=non_edit_filter))
        self.application.add_handler(CommandHandler("info", self.info, filters=non_edit_filter))
        self.application.add_handler(CallbackQueryHandler(self.void_callback_handler, pattern="void"))
        self.application.add_handler(CallbackQueryHandler(self.bot_manager.session_expired))
        self.application.add_handler(CommandHandler("react", self.react, filters=non_edit_filter))
    ...
    async def info(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        Triggered by `/info`.
        """
        if update.effective_chat.type != telegram.Chat.PRIVATE:  # Group message
    ...
    def info_group(self, update):
        ...
        links = self.db.get_chat_assoc(master_uid=etm_utils.chat_id_to_str(self.channel_id, update.effective_message.chat_id))
        if links:  # Linked chat
            msg = self._("The group {group_name} ({group_id}) is linked to:").format(
                group_name=update.effective_message.chat.title,
                group_id=update.effective_message.chat_id)
            msg += self.build_link_chats_info_str(links)
        else:
            msg = self._("The group {group_name} ({group_id}) is not linked to any remote chat. "
                         "To link one, use /link.").format(group_name=update.effective_message.chat.title,
                                                           group_id=update.effective_message.chat_id)
        return msg
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args:  # Group binding command
            if update.effective_chat.type != telegram.Chat.PRIVATE or \
    ...
    async def react(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            await coordinator.send_status(ReactToMessage(chat=chat_obj, msg_id=message_id, reaction=reaction))
    ...
    async def error(self, update: object, context: ContextTypes.DEFAULT_TYPE):
        ...
        if "make sure that only one bot instance is running" in str(error):
            ...
            if now - self.last_poll_confliction_time < self.CONFLICTION_TIMEOUT:
                ...
                await context.bot.send_message(self.config['admins'][0], msg)
        ...
        try:
            ...
            if e.message == "Message is not modified" and update.callback_query:
                ...
                await context.bot.send_message(self.config['admins'][0],
            ...
            if isinstance(update, Update) and isinstance(update.effective_message, Message):
                await update.effective_message.reply_text(self._("This message is not processed due to poor internet environment "
                                                                  "of the server.\n"
            ...
            if isinstance(update, Update) and isinstance(update.effective_message, Message):
                old_id = ChatID(str(update.effective_message.chat_id))
                count = 0
    ...
    async def send_message(self, msg: EFBMessage) -> EFBMessage:
        return await self.slave_messages.send_message(msg)

    async def send_status(self, status: Status):
        return await self.slave_messages.send_status(status)

    async def get_message_by_id(self, chat: Chat,
    ...
    async def void_callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def stop_polling(self):
        ...
        await self.master_messages.stop_worker()
