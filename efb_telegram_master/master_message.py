...
class MasterMessageProcessor(LocaleMixin):
    ...
    def message_worker(self):
        ...
        while True:
            ...
            try:
                ...
                self.logger.exception("Failed to process Telegram update (%s).", type(e).__name__)
                if update.effective_message:
                    sync_reply_text(
                        self.bot,
                        update.effective_message,
                        self._("Unknown error has occurred while trying to process this message. See log for details.\n\n{error!r}").format(error=e),
                    )
            finally:
                self.message_queue.task_done()
    ...
    def msg(self, update: Update, context: CallbackContext):
                ...
                if thread_id and topic_destinations:
                    for dest, topic_id in topic_destinations:
                        if topic_id == thread_id:
                            ...
                            quote = reply_to_message is not None and reply_to_message.message_id != reply_to_message.message_thread_id
                            break
                    if destination is None:
        ...
        if destination is None:
            ...
            if candidates:
                ...
                self.channel.chat_binding.register_suggestions(update, candidates, TelegramChatID(update.effective_chat.id), TelegramMessageID(tg_err_msg.message_id))
            else:
                self.logger.debug("[%s] Candidate suggestions not found, give up.", mid)
                sync_reply_text(self.bot, message, self._("Error: No recipient specified.\nPlease reply to a previous message. (MS02)"), quote=True)
        else:
            return self.process_telegram_message(update, context, destination, quote=quote, edited=edited)

    def process_telegram_message(self, update: Update, context: CallbackContext, destination: EFBChannelChatIDStr, quote: bool = False, edited: Optional["MsgLog"] = None):
        """
        ...
        try:
            ...
            if self.TYPE_DICT.get(mtype, None):
                ...
                log_message = False
                raise EFBMessageTypeNotSupported(self._("{type_name} messages are not supported by EFB Telegram Master channel.").format(type_name=mtype.name))
            ...
            if m.type not in coordinator.slaves[channel].supported_message_types:
                self.logger.info("[%s] Message type %s is not supported by channel %s", message_id, m.type.name, channel)
                raise EFBMessageTypeNotSupported(
                    self._("{type_name} messages are not supported by slave channel {channel_name}.").format(type_name=m.type.name, channel_name=coordinator.slaves[channel].channel_name)
                )
            ...
            if edited:
                ...
                if text.startswith(self.DELETE_FLAG):
                    coordinator.send_status(MessageRemoval(source_channel=self.channel, destination_channel=coordinator.slaves[channel], message=m))
            ...
            if mtype is TGMsgType.Text:
                ...
                m.text = "%s - %s\n%s" % (message.audio.title, message.audio.performer, msg_md_caption)
                ...
                m.attributes = LocationAttribute(message.location.latitude, message.location.longitude)
    ...
    def attach_target_message(self, tg_msg: Message, etm_msg: ETMMsg, channel: ModuleID) -> ETMMsg:
        ...
        if target_channel != channel:
            self.logger.error("[%s] Quoted message is sent to channel %s, but this message is sent to %s, give up quoting.", tg_msg.message_id, target_channel, channel)
            return etm_msg
        target_msg: ETMMsg = target_log.build_etm_msg(self.chat_manager, recur=False)
        target_msg.target = None
        etm_msg.target = target_msg

        self.logger.debug("[%s] This message replies to another message of the same channel.\nChat ID: %s; Message ID: %s.", tg_msg.message_id, target_msg.chat.uid, target_msg.uid)
        return etm_msg

    def _send_cached_chat_warning(self, update: Update, cache_key: TelegramChatID, cached_dest: EFBChannelChatIDStr):
        ...
        if not self.chat_dest_cache.is_warned(str(cache_key)):
            ...
            sync_reply_text(
                self.bot,
                update.effective_message,
                self._("This message is sent to “{dest}” with quick reply feature.\n\nLearn more about how this works, how to turn this feature off, and how to stop this warning at {docs}.").format(
                    dest=dest_name, docs="https://etm.1a23.studio/"
                ),
                quote=True,
    ...
    def _check_file_download(self, file_obj: Any):
        ...
        if size and not self.channel.flag("local_tdlib_api") and size > FileSizeLimit.FILESIZE_DOWNLOAD:
            size_str = humanize.naturalsize(size)
            max_size_str = humanize.naturalsize(FileSizeLimit.FILESIZE_DOWNLOAD)
            raise EFBMessageError(self._("Attachment is too large ({size}). Maximum allowed by Telegram Bot API is {max_size}. (AT01)").format(size=size_str, max_size=max_size_str))
    ...
    def delete_message(self, update: Update, context: CallbackContext):
        ...
        if dest_channel is None:
            return self.bot.reply_error(update, self._("Module of this message ({module_id}) could not be found, or is not a slave channel.").format(module_id=etm_msg.chat.module_id))
        # noinspection PyBroadException
        try:
            coordinator.send_status(MessageRemoval(source_channel=self.channel, destination_channel=dest_channel, message=etm_msg))
        except EFBException as e:
            ...
            return sync_reply_text(self.bot, reply, self._("Failed to remove this message from remote chat.\n\n{error!s}").format(error=e))
        except Exception as e:
            self.logger.exception(
                "Failed to remove Telegram message %s.%s from remote chat (%s).",
                reply.chat_id,
    ...
    def unsupported_message(self, update: Update, context: CallbackContext):
        ...
        self.bot.reply_error(update, self._("{type_name} messages are not supported by EFB Telegram Master channel.").format(type_name=message_type.name))
