...
from typing import Any, Dict, List, MutableSequence, Optional, Pattern, Union
...
class ETMChatMixin(ETMBaseChatMixin, Chat, ABC):
    ...
    def last_message_time(self) -> datetime:
        ...
        if self._last_message_time is None or now - self._last_message_time_query > self.LAST_MESSAGE_QUERY_TIMEOUT_MS / 1000:
            msg_log = self.db.msglogs.get_last_message(slave_chat_id=utils.chat_id_to_str(chat=self))
    ...
    def pickle(self) -> bytes:
        from efb_telegram_master.chat.chat_codec import pickle_chat
